...
const HullGeometry: React.FC = () => {
  ...
  // --- STYLES ---
  const inputClass =
    "w-full px-4 py-2.5 bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all placeholder-gray-400 font-mono";
  const labelClass =
    "block mb-1 text-xs font-semibold text-gray-500 uppercase tracking-wide flex justify-between";
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const hullRes = await fetch(
          `http://127.0.0.1:5000/api/gainputs/${gaInputId}/hull`,
          {
            headers: { Authorization: `Bearer ${token}` },
          },
        );
        ...
        if (hullRes.ok) {
          ...
          setFormData({
            ...
            length_between_perpendiculars:
              hullData.length_between_perpendiculars,
          ...
          if (projData?.vessel) {
            setFormData((prev) => ({
              ...
              length_between_perpendiculars: projData.vessel.loa
                ? (projData.vessel.loa * 0.95).toFixed(2)
                : "",
            }));
          }
        }
  ...
  const handleSave = async () => {
    ...
    if (!formData.length_overall || !formData.breadth_moulded) {
      setError("Principal Dimensions missing.");
      setIsSaving(false);
      return;
    }
    ...
    const payload = {
      ...
      length_between_perpendiculars: parseFloat(
        formData.length_between_perpendiculars,
      ),
      ...
      waterplane_coefficient:
        parseFloat(formData.waterplane_coefficient) || null,
      parallel_midbody_length:
        parseFloat(formData.parallel_midbody_length) || 0,
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(res.error || "Failed to save hull data");
      }

      // 2. TRIGGER GENERATION (The magic step)
      // We notify user we are generating
      // You might want a separate loading state text like "Generating Design..."
      console.log("Inputs Saved. Generating Design...");

      const genResponse = await fetch(
        "http://127.0.0.1:5000/api/generation/generate-rsv",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          ...
          body: JSON.stringify({ project_id: id }), // 'id' from useParams
        },
      );
      ...
      if (genResponse.ok) {
        ...
        alert(
          "Inputs saved, but Design Generation failed. You can try again from the project page.",
        );
        navigate(`/projects/${id}`);
      }
    } catch (err: any) {
    ...
    <div className="p-8 bg-gray-50 min-h-screen font-sans">
      <div className="max-w-6xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <button
            onClick={() => navigate(-1)}
            className="flex items-center gap-2 text-gray-500 font-bold text-xs hover:text-blue-600"
          >
          ...
          <button
            onClick={handleSave}
            disabled={isSaving}
            className="flex items-center gap-2 bg-blue-600 text-white px-6 py-2.5 rounded-lg font-semibold hover:bg-blue-700 disabled:opacity-50 transition-all"
          >
            {isSaving ? (
              <Loader2 className="animate-spin" size={18} />
            ) : (
              <>
                {isEditMode ? "UPDATE" : "SAVE"} & FINISH{" "}
        ...
        <div className="space-y-6">
          ...
          <section className="bg-white p-8 rounded-xl border border-gray-200 shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              ...
              <InputField
                ...
                value={formData.length_between_perpendiculars}
                onChange={handleChange}
                unit="m"
              />
              <InputField
                ...
                value={formData.breadth_moulded}
                onChange={handleChange}
                unit="m"
              />
              <InputField
          ...
          <section className="bg-white p-8 rounded-xl border border-gray-200 shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              <InputField
                label="Block Coeff (Cb)"
                name="block_coefficient"
                value={formData.block_coefficient}
                onChange={handleChange}
              />
              <InputField
                label="Prismatic Coeff (Cp)"
                name="prismatic_coefficient"
                value={formData.prismatic_coefficient}
                onChange={handleChange}
              />
              <InputField
                label="Midship Coeff (Cm)"
                name="midship_coefficient"
                value={formData.midship_coefficient}
                onChange={handleChange}
              />
              <InputField
                label="Waterplane Coeff (Cwp)"
                name="waterplane_coefficient"
                value={formData.waterplane_coefficient}
                onChange={handleChange}
              />
            </div>
          </section>
          ...
          <section className="bg-white p-8 rounded-xl border border-gray-200 shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-6 mb-8">
              <InputField
                ...
                value={formData.entrance_length}
                onChange={handleChange}
                unit="m"
              />
              <InputField
                label="Parallel Midbody"
                name="parallel_midbody_length"
                value={formData.parallel_midbody_length}
                onChange={handleChange}
                unit="m"
              />
              <InputField
                label="Run Length"
                name="run_length"
                value={formData.run_length}
                onChange={handleChange}
                unit="m"
              />
              <InputField
                label="Bow Rake Angle"
                name="bow_rake_angle"
                value={formData.bow_rake_angle}
                onChange={handleChange}
                unit="deg"
              />
              <InputField
                label="Stern Rake Angle"
                name="stern_rake_angle"
                value={formData.stern_rake_angle}
                onChange={handleChange}
                unit="deg"
              />
            </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className={inputClass}
                    >
                      <option value="AFT_TO_FWD">Aft → Fwd</option>
                      <option value="FWD_TO_AFT">Fwd → Aft</option>
                    </select>
                  </div>
                </div>
              </div>
...
const InputField = ({ label, name, value, onChange, unit }: any) => (
  <div className="space-y-2">
    <label className="block mb-1 text-xs font-semibold text-gray-500 uppercase tracking-wide flex justify-between">
      {label}
      {unit && (
        <span className="text-blue-600 font-mono text-[10px] bg-blue-50 px-1.5 py-0.5 rounded border border-blue-100">
          {unit}
        </span>
      )}
    </label>
    <input
      ...
      type="number"
      step="0.01"
      className="w-full px-4 py-2.5 bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all placeholder-gray-400 font-mono"
    />
  </div>
);
